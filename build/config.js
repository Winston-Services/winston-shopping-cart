var avatarImage =
  "data:image/png;base64,";

var productList = [
  {
    id: "02c84d0e-9c9d-4837-ada8-aad7d8345582",
    price: "29.99",
    title: "Winston Water Bottles",
    shortDescription:
      "When on the go Winston has you covered with this sporty water bottle.",
    description:
      "While on the go this sports water bottle is sure to help you when your thurst hits, just fill it with your favorite bevereages.",
    downloadable: true,
    thumbnail: avatarImage,
    images: [
      {
        src: "../assets/products/images/Winston_Waterbottle.png",
        alt: "Winston take Ahwa for a walk. Blue Sports waterbottle."
      }
    ],
    category: "Sports Gear",
    subcategory: "Water Bottles",
    overview:
      "Consequat culpa voluptate Lorem dolore aliquip sit pariatur. In do tempor reprehenderit sint excepteur labore est duis cillum. Do excepteur anim dolore ad nostrud fugiat mollit nisi. Occaecat deserunt consequat minim minim Lorem magna ex laborum reprehenderit nulla irure cillum. Deserunt excepteur sit enim fugiat exercitation ullamco elit tempor qui nulla cillum. Minim laboris pariatur eiusmod et enim incididunt occaecat cupidatat sint reprehenderit qui. Est incididunt ex dolor in enim fugiat proident pariatur officia aliquip duis.",
    reviews: [
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 5
      },
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 3
      },
      {
        comment: "Can't get it to work.",
        commenterName: "",
        commenterEmail: "",
        rating: 1
      },
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 5
      }
    ],
    tags: ["sports", "waterbottle", "winston"],
    shipsFrom: {
      address_1: "",
      address_2: "",
      address_3: "",
      address_4: ""
    },
    manufactures: [
      {
        name: "",
        address_1: "",
        address_2: "",
        address_3: "",
        address_4: ""
      }
    ],
    attributes: [{}],
    options: [
      {
        isDefault: true,
        name: "Duration",
        type: "ChooseOne",
        options: [
          {
            name: "Annual Members Role",
            value: "annual",
            label: "Annual Membership",
            acceptCurrencies: [
              "USDC",
              "BNB",
              "WBNB",
              "BUSD",
              "RICKLE",
              "WINSTON"
            ],
            price: "1.00",
            priceInUSD: "1.00"
          },
          {
            name: "Trial Members Role",
            value: "trial",
            label: "Trial Membership",
            acceptCurrencies: ["RICKLE", "WINSTON"],
            price: "1.60",
            priceInUSD: "1.60"
          }
        ]
      }
    ]
  },
  {
    id: "b8eb7e3e-1619-4014-b4cd-2212fa191ea5",
    title: "Winston Water Bottles",
    price: "29.99",
    shortDescription:
      "When learning with Winston this sporty water bottle can keep you supplied with your favorite beverages..",
    description:
      "While studing at the Winston Academy this sports water bottle is sure to help you when your thurst hits, just fill it with your favorite bevereages.",
    downloadable: true,
    thumbnail: avatarImage,
    images: [
      {
        src: "../assets/products/images/Winston_Lessons_Waterbottle.png",
        alt:
          "Winston teaches blockchain to the common person. Blue Sports waterbottle."
      }
    ],
    category: "Sports Gear",
    subcategory: "Water Bottles",
    overview:
      "Consequat culpa voluptate Lorem dolore aliquip sit pariatur. In do tempor reprehenderit sint excepteur labore est duis cillum. Do excepteur anim dolore ad nostrud fugiat mollit nisi. Occaecat deserunt consequat minim minim Lorem magna ex laborum reprehenderit nulla irure cillum. Deserunt excepteur sit enim fugiat exercitation ullamco elit tempor qui nulla cillum. Minim laboris pariatur eiusmod et enim incididunt occaecat cupidatat sint reprehenderit qui. Est incididunt ex dolor in enim fugiat proident pariatur officia aliquip duis.",
    reviews: [
      {
        comment:
          "What a wonderful product. You can't go wrong when you get one of these bottles. They last forever and a perfect for any trip.",
        commenterName: "Geana Lopez",
        commenterEmail: "glopez1944@domain.tld",
        rating: 5
      },
      {
        comment: "Blog burns fried pies.",
        commenterName: "JoAnne Marie",
        commenterEmail: "JoJoAM@domain.tld",
        rating: 3
      },
      {
        comment:
          "Orange Cat, Brown Cat, any Cat will do when you want to have a cat.",
        commenterName: "",
        commenterEmail: "",
        rating: 1
      },
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 5
      }
    ],
    tags: ["sports", "waterbottle", "winston"],
    shipsFrom: {
      address_1: "",
      address_2: "",
      address_3: "",
      address_4: ""
    },
    manufactures: [
      {
        name: "",
        address_1: "",
        address_2: "",
        address_3: "",
        address_4: ""
      }
    ],
    attributes: [{}],
    options: [
      {
        isDefault: true,
        name: "Duration",
        type: "ChooseOne",
        options: [
          {
            name: "Annual Members Role",
            value: "annual",
            label: "Annual Membership",
            acceptCurrencies: [
              "USDC",
              "BNB",
              "WBNB",
              "BUSD",
              "RICKLE",
              "WINSTON"
            ],
            price: "1.00",
            priceInUSD: "1.00"
          },
          {
            name: "Trial Members Role",
            value: "trial",
            label: "Trial Membership",
            acceptCurrencies: ["RICKLE", "WINSTON"],
            price: "1.60",
            priceInUSD: "1.60"
          }
        ]
      }
    ]
  },
  {
    id: "bf4cc44f-6ce3-4d81-9744-1f00f8b24148",
    title: "Concept Art Blanket",
    price: "109.99",
    shortDescription:
      "Collect these unique concept art blankets of the Winston in Style NFT Series.",
    description:
      "This 50x50in fleece blanket is decorated with one of the unique concept art NFT's from the Winston in Style NFT series. Collect them all for the complete collection to keep you warn all winter long.",
    downloadable: false,
    downloadUrlRequest: "", //download url proof request.
    thumbnail: avatarImage,
    images: [
      {
        src: "../assets/products/images/Winston_Concept_Art_NFT_Blanket.png",
        alt: "Winston Services Placeholder"
      }
    ],
    category: "Houseware",
    subcategory: "Blankets",
    overview:
      "Consequat culpa voluptate Lorem dolore aliquip sit pariatur. In do tempor reprehenderit sint excepteur labore est duis cillum. Do excepteur anim dolore ad nostrud fugiat mollit nisi. Occaecat deserunt consequat minim minim Lorem magna ex laborum reprehenderit nulla irure cillum. Deserunt excepteur sit enim fugiat exercitation ullamco elit tempor qui nulla cillum. Minim laboris pariatur eiusmod et enim incididunt occaecat cupidatat sint reprehenderit qui. Est incididunt ex dolor in enim fugiat proident pariatur officia aliquip duis.",
    reviews: [
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 5
      },
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 3
      },
      {
        comment: "Can't get it to work.",
        commenterName: "",
        commenterEmail: "",
        rating: 1
      },
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 5
      }
    ],
    tags: ["blankets", "fleece", "winston", "concept-art"],
    shipsFrom: {
      address_1: "",
      address_2: "",
      address_3: "",
      address_4: ""
    },
    manufactures: [
      {
        name: "",
        details: "",
        address_1: "",
        address_2: "",
        address_3: "",
        address_4: ""
      }
    ],
    attributes: [
      {
        color: "blue"
      },
      {
        color: "gold"
      }
    ],
    options: [
      {
        isDefault: true,
        name: "Duration",
        type: "ChooseOne",
        options: [
          {
            name: "Annual Members Role",
            value: "annual",
            label: "Annual Membership",
            acceptCurrencies: [
              "USDC",
              "BNB",
              "WBNB",
              "BUSD",
              "RICKLE",
              "WINSTON"
            ],
            price: "1.00",
            priceInUSD: "1.00"
          },
          {
            name: "Trial Members Role",
            value: "trial",
            label: "Trial Membership",
            acceptCurrencies: ["RICKLE", "WINSTON"],
            price: "1.60",
            priceInUSD: "1.60"
          }
        ]
      }
    ]
  },
  {
    id: "95f2e9f5-332c-4065-82fc-52ed327b930e",
    title: "Winston T-Shirts",
    price: "26.99",
    shortDescription:
      "Winston T-Shirts for when you just want to be comfortable.",
    description:
      "Purchase your members role for the rickle server and link your account to get the role. This is a yearly membership and does not auto renew.",
    downloadable: true,
    thumbnail: avatarImage,
    images: [
      {
        src: "../assets/products/images/White_T_Gold_Logo.png",
        alt: "Winston Services Placeholder"
      }
    ],
    category: "Clothing",
    subcategory: "T-Shirts",
    overview:
      "Consequat culpa voluptate Lorem dolore aliquip sit pariatur. In do tempor reprehenderit sint excepteur labore est duis cillum. Do excepteur anim dolore ad nostrud fugiat mollit nisi. Occaecat deserunt consequat minim minim Lorem magna ex laborum reprehenderit nulla irure cillum. Deserunt excepteur sit enim fugiat exercitation ullamco elit tempor qui nulla cillum. Minim laboris pariatur eiusmod et enim incididunt occaecat cupidatat sint reprehenderit qui. Est incididunt ex dolor in enim fugiat proident pariatur officia aliquip duis.",
    reviews: [
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 5
      },
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 3
      },
      {
        comment: "Can't get it to work.",
        commenterName: "",
        commenterEmail: "",
        rating: 1
      },
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 5
      }
    ],
    tags: ["subscriptions", "membership", "rickle"],
    shipsFrom: {
      address_1: "",
      address_2: "",
      address_3: "",
      address_4: ""
    },
    manufactures: [
      {
        name: "",
        address_1: "",
        address_2: "",
        address_3: "",
        address_4: ""
      }
    ],
    attributes: [{}],
    options: [
      {
        isDefault: true,
        name: "Duration",
        type: "ChooseOne",
        options: [
          {
            name: "Annual Members Role",
            value: "annual",
            label: "Annual Membership",
            acceptCurrencies: [
              "USDC",
              "BNB",
              "WBNB",
              "BUSD",
              "RICKLE",
              "WINSTON"
            ],
            price: "1.00",
            priceInUSD: "1.00"
          },
          {
            name: "Trial Members Role",
            value: "trial",
            label: "Trial Membership",
            acceptCurrencies: ["RICKLE", "WINSTON"],
            price: "1.60",
            priceInUSD: "1.60"
          }
        ]
      }
    ]
  },
  {
    id: "66ed4cea-feae-4bde-8615-fb3d6244606f",
    title: "Winston T-Shirts",
    price: "26.99",
    shortDescription:
      "Winston T-Shirts for when you just want to be comfortable.",
    description:
      "Purchase your members role for the rickle server and link your account to get the role. This is a yearly membership and does not auto renew.",
    downloadable: true,
    thumbnail: avatarImage,
    images: [
      {
        src: "../assets/products/images/White_T_Blue_Logo.png",
        alt: "Winston Services Placeholder"
      }
    ],
    category: "Clothing",
    subcategory: "T-Shirts",
    overview:
      "Consequat culpa voluptate Lorem dolore aliquip sit pariatur. In do tempor reprehenderit sint excepteur labore est duis cillum. Do excepteur anim dolore ad nostrud fugiat mollit nisi. Occaecat deserunt consequat minim minim Lorem magna ex laborum reprehenderit nulla irure cillum. Deserunt excepteur sit enim fugiat exercitation ullamco elit tempor qui nulla cillum. Minim laboris pariatur eiusmod et enim incididunt occaecat cupidatat sint reprehenderit qui. Est incididunt ex dolor in enim fugiat proident pariatur officia aliquip duis.",
    reviews: [
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 5
      },
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 3
      },
      {
        comment: "Can't get it to work.",
        commenterName: "",
        commenterEmail: "",
        rating: 1
      },
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 5
      }
    ],
    tags: ["subscriptions", "membership", "rickle"],
    shipsFrom: {
      address_1: "",
      address_2: "",
      address_3: "",
      address_4: ""
    },
    manufactures: [
      {
        name: "",
        address_1: "",
        address_2: "",
        address_3: "",
        address_4: ""
      }
    ],
    attributes: [{}],
    options: [
      {
        isDefault: true,
        name: "Duration",
        type: "ChooseOne",
        options: [
          {
            name: "Annual Members Role",
            value: "annual",
            label: "Annual Membership",
            acceptCurrencies: [
              "USDC",
              "BNB",
              "WBNB",
              "BUSD",
              "RICKLE",
              "WINSTON"
            ],
            price: "1.00",
            priceInUSD: "1.00"
          },
          {
            name: "Trial Members Role",
            value: "trial",
            label: "Trial Membership",
            acceptCurrencies: ["RICKLE", "WINSTON"],
            price: "1.60",
            priceInUSD: "1.60"
          }
        ]
      }
    ]
  },
  {
    id: "5d3e3faa-179b-439c-ae4a-f8d25f18d3d9",
    title: "Membership Role",
    shortDescription: "Get a yearly membership role.",
    description:
      "Purchase your members role for the rickle server and link your account to get the role. This is a yearly membership and does not auto renew.",
    downloadable: true,
    thumbnail: avatarImage,
    images: [
      {
        src: "../assets/products/images/White_Blue_Beenie_Blue_Gold_Logo.png",
        alt: "Winston Services Placeholder"
      }
    ],
    category: "Clothing",
    subcategory: "Beenie's",
    overview:
      "Consequat culpa voluptate Lorem dolore aliquip sit pariatur. In do tempor reprehenderit sint excepteur labore est duis cillum. Do excepteur anim dolore ad nostrud fugiat mollit nisi. Occaecat deserunt consequat minim minim Lorem magna ex laborum reprehenderit nulla irure cillum. Deserunt excepteur sit enim fugiat exercitation ullamco elit tempor qui nulla cillum. Minim laboris pariatur eiusmod et enim incididunt occaecat cupidatat sint reprehenderit qui. Est incididunt ex dolor in enim fugiat proident pariatur officia aliquip duis.",
    reviews: [
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 5
      },
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 3
      },
      {
        comment: "Can't get it to work.",
        commenterName: "",
        commenterEmail: "",
        rating: 1
      },
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 5
      }
    ],
    tags: ["winston", "beenie", "winter"],
    shipsFrom: {
      address_1: "",
      address_2: "",
      address_3: "",
      address_4: ""
    },
    manufactures: [
      {
        name: "VistaPrint",
        address_1: "https://vistaprint.com",
        address_2: "",
        address_3: "",
        address_4: ""
      }
    ],
    options: [
      {
        isDefault: true,
        name: "Duration",
        type: "ChooseOne",
        options: [
          {
            name: "Annual Members Role",
            value: "annual",
            label: "Annual Membership",
            attributes: {
              size: [
                "xs:Extra Small",
                "sm:Small",
                "m:Medium",
                "l:Large",
                "xl:Extra Large",
                "xxl:Double Extra Large",
                "xxxl:Tripple Extra Large"
              ],
              color: ["red:red", "blue:blue", "gold:gold", "black:black"]
            },
            acceptCurrencies: {
              USDC: "USDC",
              BNB: "BNB",
              WBNB: "WBNB",
              BUSD: "BUSD",
              RICKLE: "RICKLE",
              WINSTON: "WINSTON"
            },
            price: "1.00",
            priceInUSD: "1.00"
          },
          {
            name: "Trial Members Role",
            value: "trial",
            label: "Trial Membership",
            acceptCurrencies: ["RICKLE", "WINSTON"],
            price: "1.60",
            priceInUSD: "1.60"
          }
        ]
      }
    ]
  },
  {
    id: "0fbaf2f8-23cb-4f83-b8d1-d6397c6b10d4",
    title: "Winston Beenie's",
    shortDescription:
      "Keep in style during the colder day with this stylish Winston Beenie.",
    description:
      "Purchase your members role for the rickle server and link your account to get the role. This is a yearly membership and does not auto renew.",
    downloadable: true,
    thumbnail: avatarImage,
    images: [
      {
        src: "../assets/products/images/White_Beenie_Blue_Logo.png",
        alt: "Winston Services Placeholder"
      }
    ],
    category: "Clothing",
    subcategory: "Beenie's",
    overview:
      "Consequat culpa voluptate Lorem dolore aliquip sit pariatur. In do tempor reprehenderit sint excepteur labore est duis cillum. Do excepteur anim dolore ad nostrud fugiat mollit nisi. Occaecat deserunt consequat minim minim Lorem magna ex laborum reprehenderit nulla irure cillum. Deserunt excepteur sit enim fugiat exercitation ullamco elit tempor qui nulla cillum. Minim laboris pariatur eiusmod et enim incididunt occaecat cupidatat sint reprehenderit qui. Est incididunt ex dolor in enim fugiat proident pariatur officia aliquip duis.",
    reviews: [
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 5
      },
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 3
      },
      {
        comment: "Can't get it to work.",
        commenterName: "",
        commenterEmail: "",
        rating: 1
      },
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 5
      }
    ],
    tags: ["beenie", "winston"],
    shipsFrom: {
      address_1: "",
      address_2: "",
      address_3: "",
      address_4: ""
    },
    manufactures: [
      {
        name: "",
        address_1: "",
        address_2: "",
        address_3: "",
        address_4: ""
      }
    ],
    attributes: [{}],
    options: [
      {
        isDefault: true,
        name: "Duration",
        type: "ChooseOne",
        options: [
          {
            name: "Annual Members Role",
            value: "annual",
            label: "Annual Membership",
            acceptCurrencies: [
              "USDC",
              "BNB",
              "WBNB",
              "BUSD",
              "RICKLE",
              "WINSTON"
            ],
            price: "1.00",
            priceInUSD: "1.00"
          },
          {
            name: "Trial Members Role",
            value: "trial",
            label: "Trial Membership",
            acceptCurrencies: ["RICKLE", "WINSTON"],
            price: "1.60",
            priceInUSD: "1.60"
          }
        ]
      }
    ]
  },
  {
    id: "98655bc1-c70f-4d69-8ada-452703fc5d15",
    title: "Membership Role",
    shortDescription: "Get a yearly membership role.",
    description:
      "Purchase your members role for the rickle server and link your account to get the role. This is a yearly membership and does not auto renew.",
    downloadable: true,
    thumbnail: avatarImage,
    images: [
      {
        src: "../assets/products/images/Pillow_Winston_Walk_Ahwa.png",
        alt: "Winston Services Placeholder"
      }
    ],
    category: "Houseware",
    subcategory: "Pillows",
    overview:
      "Consequat culpa voluptate Lorem dolore aliquip sit pariatur. In do tempor reprehenderit sint excepteur labore est duis cillum. Do excepteur anim dolore ad nostrud fugiat mollit nisi. Occaecat deserunt consequat minim minim Lorem magna ex laborum reprehenderit nulla irure cillum. Deserunt excepteur sit enim fugiat exercitation ullamco elit tempor qui nulla cillum. Minim laboris pariatur eiusmod et enim incididunt occaecat cupidatat sint reprehenderit qui. Est incididunt ex dolor in enim fugiat proident pariatur officia aliquip duis.",
    reviews: [
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 5
      },
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 3
      },
      {
        comment: "Can't get it to work.",
        commenterName: "",
        commenterEmail: "",
        rating: 1
      },
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 5
      }
    ],
    tags: ["subscriptions", "membership", "rickle"],
    shipsFrom: {
      address_1: "",
      address_2: "",
      address_3: "",
      address_4: ""
    },
    manufactures: [
      {
        name: "",
        address_1: "",
        address_2: "",
        address_3: "",
        address_4: ""
      }
    ],
    attributes: [{}],
    options: [
      {
        isDefault: true,
        name: "Duration",
        type: "ChooseOne",
        options: [
          {
            name: "Annual Members Role",
            value: "annual",
            label: "Annual Membership",
            acceptCurrencies: [
              "USDC",
              "BNB",
              "WBNB",
              "BUSD",
              "RICKLE",
              "WINSTON"
            ],
            price: "1.00",
            priceInUSD: "1.00"
          },
          {
            name: "Trial Members Role",
            value: "trial",
            label: "Trial Membership",
            acceptCurrencies: ["RICKLE", "WINSTON"],
            price: "1.60",
            priceInUSD: "1.60"
          }
        ]
      }
    ]
  },
  {
    id: "42a21dbe-8157-45b3-adb5-2f57566106e4",
    title: "Membership Role",
    shortDescription: "Get a yearly membership role.",
    description:
      "Purchase your members role for the rickle server and link your account to get the role. This is a yearly membership and does not auto renew.",
    downloadable: true,
    thumbnail: avatarImage,
    images: [
      {
        src: "../assets/products/images/Pillow_Winston_In_Gold.png",
        alt: "Winston Services Placeholder"
      }
    ],
    category: "Houseware",
    subcategory: "Pillows",
    overview:
      "Consequat culpa voluptate Lorem dolore aliquip sit pariatur. In do tempor reprehenderit sint excepteur labore est duis cillum. Do excepteur anim dolore ad nostrud fugiat mollit nisi. Occaecat deserunt consequat minim minim Lorem magna ex laborum reprehenderit nulla irure cillum. Deserunt excepteur sit enim fugiat exercitation ullamco elit tempor qui nulla cillum. Minim laboris pariatur eiusmod et enim incididunt occaecat cupidatat sint reprehenderit qui. Est incididunt ex dolor in enim fugiat proident pariatur officia aliquip duis.",
    reviews: [
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 5
      },
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 3
      },
      {
        comment: "Can't get it to work.",
        commenterName: "",
        commenterEmail: "",
        rating: 1
      },
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 5
      }
    ],
    tags: ["subscriptions", "membership", "rickle"],
    shipsFrom: {
      address_1: "",
      address_2: "",
      address_3: "",
      address_4: ""
    },
    manufactures: [
      {
        name: "",
        address_1: "",
        address_2: "",
        address_3: "",
        address_4: ""
      }
    ],
    attributes: [{}],
    options: [
      {
        isDefault: true,
        name: "Duration",
        type: "ChooseOne",
        options: [
          {
            name: "Annual Members Role",
            value: "annual",
            label: "Annual Membership",
            acceptCurrencies: [
              "USDC",
              "BNB",
              "WBNB",
              "BUSD",
              "RICKLE",
              "WINSTON"
            ],
            price: "1.00",
            priceInUSD: "1.00"
          },
          {
            name: "Trial Members Role",
            value: "trial",
            label: "Trial Membership",
            acceptCurrencies: ["RICKLE", "WINSTON"],
            price: "1.60",
            priceInUSD: "1.60"
          }
        ]
      }
    ]
  },
  {
    id: "f925a287-1913-48cc-80c9-a522cc5d5a06",
    title: "Membership Role",
    shortDescription: "Get a yearly membership role.",
    description:
      "Purchase your members role for the rickle server and link your account to get the role. This is a yearly membership and does not auto renew.",
    downloadable: true,
    thumbnail: avatarImage,
    images: [
      {
        src: "../assets/products/images/Pillow_Blue_Gold_Logo.png",
        alt: "Winston Services Placeholder"
      }
    ],
    category: "Houseware",
    subcategory: "Pillows",
    overview:
      "Consequat culpa voluptate Lorem dolore aliquip sit pariatur. In do tempor reprehenderit sint excepteur labore est duis cillum. Do excepteur anim dolore ad nostrud fugiat mollit nisi. Occaecat deserunt consequat minim minim Lorem magna ex laborum reprehenderit nulla irure cillum. Deserunt excepteur sit enim fugiat exercitation ullamco elit tempor qui nulla cillum. Minim laboris pariatur eiusmod et enim incididunt occaecat cupidatat sint reprehenderit qui. Est incididunt ex dolor in enim fugiat proident pariatur officia aliquip duis.",
    reviews: [
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 5
      },
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 3
      },
      {
        comment: "Can't get it to work.",
        commenterName: "",
        commenterEmail: "",
        rating: 1
      },
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 5
      }
    ],
    tags: ["subscriptions", "membership", "rickle"],
    shipsFrom: {
      address_1: "",
      address_2: "",
      address_3: "",
      address_4: ""
    },
    manufactures: [
      {
        name: "",
        address_1: "",
        address_2: "",
        address_3: "",
        address_4: ""
      }
    ],
    attributes: [{}],
    options: [
      {
        isDefault: true,
        name: "Duration",
        type: "ChooseOne",
        options: [
          {
            name: "Annual Members Role",
            value: "annual",
            label: "Annual Membership",
            acceptCurrencies: [
              "USDC",
              "BNB",
              "WBNB",
              "BUSD",
              "RICKLE",
              "WINSTON"
            ],
            price: "1.00",
            priceInUSD: "1.00"
          },
          {
            name: "Trial Members Role",
            value: "trial",
            label: "Trial Membership",
            acceptCurrencies: ["RICKLE", "WINSTON"],
            price: "1.60",
            priceInUSD: "1.60"
          }
        ]
      }
    ]
  },
  {
    id: "a2918e7e-d78b-4b05-8e94-d790a1b7df2e",
    title: "Membership Role",
    shortDescription: "Get a yearly membership role.",
    description:
      "Purchase your members role for the rickle server and link your account to get the role. This is a yearly membership and does not auto renew.",
    downloadable: true,
    thumbnail: avatarImage,
    images: [
      {
        src: "../assets/products/images/Pillow_Ahwa.png",
        alt: "Winston Services Placeholder"
      }
    ],
    category: "Houseware",
    subcategory: "Pillows",
    overview:
      "Consequat culpa voluptate Lorem dolore aliquip sit pariatur. In do tempor reprehenderit sint excepteur labore est duis cillum. Do excepteur anim dolore ad nostrud fugiat mollit nisi. Occaecat deserunt consequat minim minim Lorem magna ex laborum reprehenderit nulla irure cillum. Deserunt excepteur sit enim fugiat exercitation ullamco elit tempor qui nulla cillum. Minim laboris pariatur eiusmod et enim incididunt occaecat cupidatat sint reprehenderit qui. Est incididunt ex dolor in enim fugiat proident pariatur officia aliquip duis.",
    reviews: [
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 5
      },
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 3
      },
      {
        comment: "Can't get it to work.",
        commenterName: "",
        commenterEmail: "",
        rating: 1
      },
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 5
      }
    ],
    tags: ["subscriptions", "membership", "rickle"],
    shipsFrom: {
      address_1: "",
      address_2: "",
      address_3: "",
      address_4: ""
    },
    manufactures: [
      {
        name: "",
        address_1: "",
        address_2: "",
        address_3: "",
        address_4: ""
      }
    ],
    attributes: [{}],
    options: [
      {
        isDefault: true,
        name: "Duration",
        type: "ChooseOne",
        options: [
          {
            name: "Annual Members Role",
            value: "annual",
            label: "Annual Membership",
            acceptCurrencies: [
              "USDC",
              "BNB",
              "WBNB",
              "BUSD",
              "RICKLE",
              "WINSTON"
            ],
            price: "1.00",
            priceInUSD: "1.00"
          },
          {
            name: "Trial Members Role",
            value: "trial",
            label: "Trial Membership",
            acceptCurrencies: ["RICKLE", "WINSTON"],
            price: "1.60",
            priceInUSD: "1.60"
          }
        ]
      }
    ]
  },
  {
    id: "73a9d144-6ca5-43c9-a2e7-911d2448034d",
    title: "Membership Role",
    shortDescription: "Get a yearly membership role.",
    description:
      "Purchase your members role for the rickle server and link your account to get the role. This is a yearly membership and does not auto renew.",
    downloadable: true,
    thumbnail: avatarImage,
    images: [
      {
        src: "../assets/products/images/Black_Mouse_White_Logo.png",
        alt: "Winston Services Placeholder"
      }
    ],
    category: "Electronics",
    subcategory: "Mouse",
    overview:
      "Consequat culpa voluptate Lorem dolore aliquip sit pariatur. In do tempor reprehenderit sint excepteur labore est duis cillum. Do excepteur anim dolore ad nostrud fugiat mollit nisi. Occaecat deserunt consequat minim minim Lorem magna ex laborum reprehenderit nulla irure cillum. Deserunt excepteur sit enim fugiat exercitation ullamco elit tempor qui nulla cillum. Minim laboris pariatur eiusmod et enim incididunt occaecat cupidatat sint reprehenderit qui. Est incididunt ex dolor in enim fugiat proident pariatur officia aliquip duis.",
    reviews: [
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 5
      },
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 3
      },
      {
        comment: "Can't get it to work.",
        commenterName: "",
        commenterEmail: "",
        rating: 1
      },
      {
        comment: "Love the rewards",
        commenterName: "",
        commenterEmail: "",
        rating: 5
      }
    ],
    tags: ["mouse", "computer", "electronics"],
    shipsFrom: {
      address_1: "",
      address_2: "",
      address_3: "",
      address_4: ""
    },
    manufactures: [
      {
        name: "",
        address_1: "",
        address_2: "",
        address_3: "",
        address_4: ""
      }
    ],
    attributes: [{}],
    options: [
      {
        isDefault: true,
        name: "Duration",
        type: "ChooseOne",
        options: [
          {
            name: "Annual Members Role",
            value: "annual",
            label: "Annual Membership",
            acceptCurrencies: [
              "USDC",
              "BNB",
              "WBNB",
              "BUSD",
              "RICKLE",
              "WINSTON"
            ],
            price: "1.00",
            priceInUSD: "1.00"
          },
          {
            name: "Trial Members Role",
            value: "trial",
            label: "Trial Membership",
            acceptCurrencies: ["RICKLE", "WINSTON"],
            price: "1.60",
            priceInUSD: "1.60"
          }
        ]
      }
    ]
  }
];

productList.constructor.prototype.getProduct = id => {};

productList.constructor.prototype.getProducts = (page, limit) => {};

productList.constructor.prototype.addProduct = data => {};

productList.constructor.prototype.updateProduct = (id, data) => {};

productList.constructor.prototype.deleteProduct = id => {};
window.config = {
  host: window.location.origin.includes("localhost:")
    ? window.location.origin
    : "http://winston.services",
  apiKey: "HH5C3ZYPGVQUT8FIVNIJXGDHQPNXUEC89U",
  admin: ["0x4Af656BBbf30900bf433C4FB7f856D2B30Ee6Fd9"],
  copyright: "Copyright © 2022 M.A.D. Computer Consulting LLC",
  productList,
  RECAPTCA_KEY: "6LdF4tkfAAAAAIlIBwHPLAH6oU9jtVIRB8Cn5FO9",
  checkoutSettings: {
    paymentTypes: ["crypto", "credit-card"],
    cryptoPaymentAddress: { bnb: "", eth: "", polygon: "", xdai: "" },
    paymentCurrencies: [
      {
        type: "credit-card",
        network: "nmi",
        currency: ["USD", "CAD"]
      },
      {
        type: "crypto",
        network: "bnb",
        currency: ["BNB", "BUSD", "USDC", "USDT", "RKL", "WIN"]
      },
      {
        type: "crypto",
        network: "eth",
        currency: ["ETH", "USDC", "RKL"]
      },
      {
        type: "crypto",
        network: "polygon",
        currency: ["MATIC", "USDC", "MIMATIC", "RKL"]
      },
      {
        type: "crypto",
        network: "xdai",
        currency: ["XDAI", "USDC", "RKL"]
      }
    ],
    defaultPaymentCurrency: {
      type: "crypto",
      network: "bnb",
      currency: "WIN"
    },
    termsAndConditions: "https://gist.com/ugedkig"
  }
};
